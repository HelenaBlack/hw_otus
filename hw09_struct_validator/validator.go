	...
	for i := 0; i < rv.NumField(); i++ {
		...
		if validateTag == "nested" {
			if fieldValue.Kind() == reflect.Struct {
				valErrs, err := validateNested(field.Name, fieldValue)
				if err != nil {
					return err
				}
...
func validateString(value, rule string) error {
	...
	case "regexp":
		...
		if err != nil {
			return fmt.Errorf("%w: %w", ErrInvalidRegexp, err)
		}
...
func validateNested(fieldName string, fieldValue reflect.Value) (ValidationErrors, error) {
	var validationErrors ValidationErrors
	...
	if err != nil {
		...
		return nil, err // Ошибка программиста
	}
	return nil, nil
}
